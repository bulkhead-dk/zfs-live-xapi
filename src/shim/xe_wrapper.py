...
try:
    from shim import sr_metadata  # noqa: E402
except ImportError:
    try:
        import sr_metadata  # type: ignore[no-redef]  # noqa: E402
    except ImportError:
...
def _xe_real(args, capture=True):
    ...
    if real is None:
        return None
    try:
        result = subprocess.check_output([real] + args, stderr=subprocess.STDOUT, close_fds=True)
...
def _invoke_cross_host_copy(src_sr_uuid, src_vdi_uuid, dest_sr_uuid, dest_host_addr, dbg):
    ...
    try:
        ...
        sys.stderr.write("xe-wrapper: zfs snapshot {} failed: {}\n".format(snap_name, e))
        return None
    ...
    try:
        ...
        recv_argv = (
            ["ssh"] + list(_SSH_OPTS) + ["root@" + dest_host_addr, "zfs", "receive", dest_zvol]
        )
    ...
    register_script = VOLUME_ROOT + "/" "org.xen.xapi.storage.zfs-live/Volume.import_existing"
    register_argv = (
        ["ssh"] + list(_SSH_OPTS) + ["root@" + dest_host_addr, register_script, "--json"]
    )
    try:
        proc = subprocess.Popen(  # pylint: disable=consider-using-with
            register_argv,
        ...
        sys.stderr.write(
            "xe-wrapper: failed to spawn ssh for " "Volume.import_existing: {}\n".format(e)
        )
        _cleanup_dest_zvol()
        return None
    if proc.returncode != 0:
...
def main(argv=None):
    ...
    if mode == "same-host":
        ...
        if not new_uuid:
            sys.stderr.write(
                "xe-wrapper: Volume.copy fast path failed; " "falling back to xe.real (sparse_dd)\n"
            )
            _passthrough(argv)
        ...
        new_uuid = _invoke_cross_host_copy(
            ...
            dest_sr,
            dest_host_addr,
            dbg="xe-wrapper:vdi-copy:{}->{}@{}".format(src_vdi, dest_sr, dest_host_addr),
        )
        if not new_uuid:
            sys.stderr.write(
                "xe-wrapper: cross-host fast path failed; " "falling back to xe.real (sparse_dd)\n"
            )
            _passthrough(argv)
