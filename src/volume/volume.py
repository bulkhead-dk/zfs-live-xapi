...
class Implementation(DefaultImplementation):
        ...
        if not thin:
            try:
                effective_copies = int(
                    zfs_operations.vol_get_property(dbg, sr_dataset, "copies")
                )
                ...
                raise util.create_storage_error(
                    ...
                    [
                        ...
                        "Available: {} bytes".format(
                            required, size, effective_copies, available
                        ),
                    ],
                )
    ...
    def destroy(self, dbg, sr, key):
        ...
        with VolumeContext(cb, sr, "w") as opq:
            with PollLock(opq, "gl", cb, 0.5):
                with cb.db_context(opq) as db:
                    ...
                    if is_snapshot:
                        snap_name = zfs_operations.find_snapshot_by_uuid(
                            dbg, sr_dataset, vdi.uuid
                        )
                        ...
                        clone_path = zfs_operations.format_zvol_name(
                            sr_dataset, vdi.uuid
                        )
                        external_deps = [
                            dep
                            for dep in zfs_operations.find_snapshot_clones(
                                dbg, snap_name
                            )
                            if dep != clone_path
                        ]
                        ...
                        vol_dependencies = []
                        for vol_snap in zfs_operations.list_zvol_snapshots(
                            dbg, vol_name
                        ):
                            ...
                            if snap_dependencies:
                                ...
                                db.insert_vdi(
                                    "clone", "", clone_uuid, clone_volume.id, False
                                )
                                clone_path = zfs_operations.format_zvol_name(
                                    sr_dataset, clone_uuid
                                )
                        ...
                        if vol_dependencies:
                            ...
                            zfs_operations.log_pool_tree(
                                dbg, "after promotions", sr_dataset
                            )
        ...
        try:
            ...
            try:
                ...
                legacy_key = hashlib.sha256(device_path.encode("utf-8")).hexdigest()[
                    :16
                ]
                qemudisk_raw.remove_cbt_metadata(dbg, sr_uri, legacy_key)
            except Exception as e:  # pylint: disable=broad-exception-caught
                log.warning(
                    "{}: legacy CBT cleanup failed " "(continuing): {}".format(dbg, e)
                )
        except ImportError:
    ...
    def stat(self, dbg, sr, key):
        ...
        with VolumeContext(cb, sr, "r") as opq:
            with cb.db_context(opq) as db:
                ...
                is_snapshot = vdi.volume.snap
                if is_snapshot:
                    vol_name = zfs_operations.find_snapshot_by_uuid(
                        dbg, sr_dataset, vdi.uuid
                    )
                    if vol_name is None:
                        raise util.create_storage_error(
                            ...
                            [
                                "Snapshot not found",
                                "ZFS snapshot %s missing from backing store"
                                % (vdi.uuid,),
                            ],
                        )
                else:
        ...
        with VolumeContext(cb, sr, "w") as opq:
            with PollLock(opq, "gl", cb, 0.5):
                with cb.db_context(opq) as db:
                    ...
                    snap_volume = db.insert_child_volume(
            ...
            try:
                _cbt_capture_at_snapshot(
                    dbg, sr, sr_dataset, parent_vdi_uuid, snap_uuid
                )
            except Exception as e:  # pylint: disable=broad-exception-caught
                log.warning(
                    "{}: CBT snapshot capture failed " "(continuing): {}".format(dbg, e)
                )
        ...
        with VolumeContext(cb, sr, "w") as opq:
            with PollLock(opq, "gl", cb, 0.5):
                with cb.db_context(opq) as db:
                    ...
                    cloned_volume = db.insert_child_volume(
                        snap_parent_id, vdi.volume.vsize
                    )
    ...
    def copy(self, dbg, sr, key, dest_sr):
        ...
        src_sr_dataset = zfs_operations.dataset_path(
            src_meta["zpool"], src_meta["dataset"]
        )
        ...
        if not dest_meta or "zpool" not in dest_meta or "dataset" not in dest_meta:
            log.debug(
                "%s: Volume.copy: dest SR %s is not zfs-live; "
                "fall back to generic copy",
                dbg,
                dest_sr,
            )
        ...
        dest_sr_dataset = zfs_operations.dataset_path(
            dest_meta["zpool"], dest_meta["dataset"]
        )
        ...
        with VolumeContext(cb, dest_sr, "w") as dest_opq:
            with PollLock(dest_opq, "gl", cb, 0.5):
                with cb.db_context(dest_opq) as db:
                    ...
                    db.insert_vdi(
                        vdi_name, vdi_description, dest_uuid, volume.id, vdi_sharable
                    )
    ...
    def set(self, dbg, sr, key, k, v):
        ...
        if k in MUTABLE_ZFS_PROPERTIES:
            ...
            vol_name = zfs_operations.format_zvol_name(sr_dataset, key)
            log.debug(
                "%s: VDI.set: applying ZFS property %s=%s on %s", dbg, k, v, vol_name
            )
    ...
    def unset(self, dbg, sr, key, k):
        ...
        if k in MUTABLE_ZFS_PROPERTIES:
            vol_name = zfs_operations.format_zvol_name(sr_dataset, key)
            log.debug(
                "%s: VDI.unset: inheriting ZFS property %s on %s", dbg, k, vol_name
            )
...
def _cbt_capture_at_snapshot(dbg, sr, sr_dataset, parent_vdi_uuid, snap_uuid):
    ...
    try:
        ...
        log.debug(
            "{}: CBT snapshot capture: qemudisk_raw not "
            "available ({}); skipping".format(dbg, e)
        )
        return
    ...
    qemudisk_raw.save_cbt_metadata(
        dbg, "file://" + sr, snap_uuid, {frozen_name: payload}
    )
