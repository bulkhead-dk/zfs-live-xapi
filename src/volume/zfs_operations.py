...
def run_zfs_command_with_retry(
    ...
    return run_zfs_command(dbg, cmd_args, error=error, simple=simple, expRc=expRc, ntries=10)
...
def vol_exists(zvol_path):
    ...
    try:
        with open(os.devnull, "w") as devnull:  # pylint: disable=unspecified-encoding
            subprocess.check_call(["zfs", "list", "-H", zvol_path], stdout=devnull, stderr=devnull)
        return True
    ...
    if create_parents:
        ...
        for parent in dataset_get_parents(dataset_name):
            if not dataset_exists(dbg, parent):
                log.debug("%s: creating parent dataset %s with mountpoint=none", dbg, parent)
...
def dataset_destroy(dbg, dataset_name, recursive=True):
    ...
    if lzc.available():
        ...
        if rc == 0:
            return
        raise xapi.InternalError(
            "lzc_destroy({}) failed: errno {} ({})".format(dataset_name, rc, lzc.errno_to_str(rc))
        )
...
def dataset_find_child_srs(dbg, pool_name, dataset_path):  # pylint: disable=redefined-outer-name
...
def create_zvol(
    ...
    has_extra_props = any(p is not None for p in (copies, compression, sync, primarycache, logbias))
    if thin and not has_extra_props:
        ...
        if lzc.create_zvol_available():
            ...
            if rc == 0:
                return
            raise xapi.InternalError(
                "lzc_create({}) failed: errno {} ({})".format(zvol_path, rc, lzc.errno_to_str(rc))
            )
...
def destroy_zvol(dbg, zvol_path):
    ...
    if lzc.available():
        ...
        if rc == 0:
            return
        raise xapi.InternalError(
            "lzc_destroy({}) failed: errno {} ({})".format(zvol_path, rc, lzc.errno_to_str(rc))
        )
...
def take_snapshot(dbg, snap_name):
    ...
    if lzc.nvlist_available():
        ...
        if rc == 0:
            return
        raise xapi.InternalError(
            "lzc_snapshot({}) failed: errno {} ({})".format(snap_name, rc, lzc.errno_to_str(rc))
        )
...
def vol_get_zfs_properties_dict(dbg, vol_name):
    ...
    keys = {}
    try:
        batch_props = ["refreservation", "volblocksize"] + sorted(MUTABLE_ZFS_PROPERTIES)
        ...
        log.debug("%s: vol_get_zfs_properties_dict: failed for %s: %s", dbg, vol_name, e)
    return keys
...
def vol_receive(dbg, pool_name, input_file):
    ...
    with open(input_file, "rb") as f:
        ...
        if proc.returncode != 0:
            raise xapi.InternalError(
                "zfs receive failed: {}".format(stderr.decode("utf-8", errors="replace"))
            )
...
def _zvol_device_is_open(zvol_path):
    ...
    try:
        ...
        try:
            subprocess.check_call(["fuser", "-s", dev_path], stdout=devnull, stderr=devnull)
...
def zvol_find_orphans(dbg, sr_dataset, known_vdi_uuids):
    ...
    for zvol_name in all_zvols:
        ...
        if _zvol_device_is_open(zvol_name):
            log.debug("%s: zvol_find_orphans: %s has open device, skipping", dbg, zvol_name)
            continue
...
def zsnap_find_orphans(dbg, sr_dataset, known_vdi_uuids):
    ...
    for snap_name in all_snaps:
        ...
        # Parent in-flight protection -- reuse zvol checks on the parent.
        if _zvol_device_is_open(parent_zvol):
            log.debug(
                "%s: zsnap_find_orphans: parent %s has open device, " "skipping snapshot %s",
                dbg,
                parent_zvol,
                snap_name,
            )
        ...
        if _zvol_has_active_datapath(parent_zvol):
            log.debug(
                "%s: zsnap_find_orphans: parent %s has active " "datapath, skipping snapshot %s",
                dbg,
                parent_zvol,
                snap_name,
            )
    ...
    for snap_name, used, creation in orphans:
        if grace_period_seconds > 0:
            try:
                ...
                log.warning(
                    "%s: orphan snapshot %s has unparseable " "creation %r -- skipping for safety",
                    dbg,
                    snap_name,
                    creation,
                )
            ...
            if age < grace_period_seconds:
                log.info(
                    "%s: orphan snapshot %s is %ds old (< grace %ds) " "-- skipping cleanup",
                    dbg,
                    snap_name,
                    age,
        ...
        try:
            ...
            log.warning(
                "%s: failed to destroy orphan snapshot %s " "(may have dependent clones): %s",
                dbg,
                snap_name,
                e,
            )
    ...
    for zvol_name, volsize, creation in orphans:
        if grace_period_seconds > 0:
            try:
                ...
                log.warning(
                    "%s: orphan %s has unparseable creation %r -- " "skipping for safety",
                    dbg,
                    zvol_name,
                    creation,
                )
