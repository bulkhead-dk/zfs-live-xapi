    ...
    if value not in valid_set:
        configuration_errors.append(
            "{} must be one of: {}, got '{}'".format(
                param, ", ".join(sorted(valid_set)), value
            )
        )
...
class Implementation(xapi.storage.api.v5.volume.SR_skeleton):
    ...
    def create(self, dbg, sr_uuid, configuration, name, description):
        log.debug(
            "{}: SR.create: config={}, sr_uuid={}".format(dbg, configuration, sr_uuid)
        )
        ...
        if adopt:
            ...
            if zfs_operations.dataset_exists(dbg, sr_dataset):
                raise util.create_storage_error(
                    ...
                    [
                        ...
                        "dataset '{}' already exists in pool '{}'".format(
            ...
            for parent in zfs_operations.dataset_get_parents(sr_dataset):
                if zfs_operations.dataset_exists(
                    dbg, parent
                ) and zfs_operations.dataset_is_sr(dbg, parent):
                    raise util.create_storage_error(
                        "SR_BACKEND_FAILURE_109",
                        [
            ...
            child_srs = zfs_operations.dataset_find_child_srs(
                dbg, pool_name, sr_dataset
            )
            if child_srs:
                raise util.create_storage_error(
                    "SR_BACKEND_FAILURE_109",
        ...
        if datapath not in ("tapdisk", "raw-qdisk"):
            raise util.create_storage_error(
                ...
                [
                    "Invalid configuration",
                    "datapath must be 'tapdisk' or 'raw-qdisk', got '{}'".format(
                        datapath
                    ),
                ],
            )
            ...
            if failed:
                raise util.create_storage_error(
                    ...
                    [
                        ...
                        "register: {}".format(
                            len(failed), len(zvols), ", ".join(failed)
                        )
                    ],
                )
    ...
    def attach(self, dbg, configuration):
        ...
        try:
            ...
            if zvols:
                log.debug(
                    "%s: SR.attach: detecting properties for %d VDI(s)", dbg, len(zvols)
                )
                for zvol_name in zvols:
        ...
        try:
            ...
            if sr_uuid:
                ...
                log.debug(
                    "%s: SR.attach: zfs-features probe skipped -- "
                    "no xcp:sr marker on %s",
                    dbg,
                    sr_dataset,
                )
    ...
    def ls(self, dbg, sr):
        ...
        with VolumeContext(cb, sr, "r") as opq:
            ...
            try:
                ...
                if orphans:
                    for zvol_name, volsize, creation in orphans:
                        log.warning(
                            "%s: SR.ls: orphan zvol detected: %s "
                            "(size=%s, created=%s)",
                            dbg,
                            zvol_name,
                            volsize,
            ...
            try:
                snap_orphans = zfs_operations.zsnap_find_orphans(
                    dbg, sr_dataset, known_uuids
                )
                if snap_orphans:
                    for snap_name, used, creation in snap_orphans:
                        log.warning(
                            "%s: SR.ls: orphan snapshot detected: %s "
                            "(used=%s, created=%s)",
                            dbg,
                            snap_name,
                            used,
            ...
            for vdi in vdis:
                ...
                if is_snapshot:
                    ...
                    if not zfs_operations.vol_exists(vol_name):
                        log.error(
                            "volume %s not found on disk for VDI %s", vol_name, vdi.uuid
                        )
                        continue
    ...
    def probe(self, dbg, configuration):
        ...
        if not zfs_operations.pool_is_imported(dbg, pool_name):
            extra = {}
            if zfs_operations.pool_exists(dbg, pool_name):
                extra["error"] = "pool '{}' exists but is not imported".format(
                    pool_name
                )
            else:
        ...
        if zfs_operations.dataset_exists(dbg, location):
            ...
            existing_srs.extend(
                zfs_operations.dataset_find_child_srs(dbg, pool_name, location)
            )
        ...
        if parent_path and not zfs_operations.dataset_exists(dbg, location):
            create_ready = False
            create_extra["error"] = "parent dataset '{}' does not exist".format(
                location
            )
        else:
            for parent in zfs_operations.dataset_get_parents(location):
                if zfs_operations.dataset_exists(
                    ...
                    create_ready = False
                    create_extra["error"] = (
                        "ancestor dataset '{}' is already an SR".format(parent)
                    )
                    break
    ...
    def _probe_sr_info(self, dbg, sr_dataset, sr_uuid):
        ...
        try:
            ...
            log.debug(
                "{}: SR.probe: meta read failed for {}: {}".format(dbg, sr_dataset, e)
            )
