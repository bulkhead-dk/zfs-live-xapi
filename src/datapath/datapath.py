...
def _get_block_device_sectors(dbg, device_path):
    ...
    try:
        ...
        log.debug("%s: failed to read sysfs sector sizes for %s: %s", dbg, device_path, e)
        return (None, None)
...
def _get_zvol_features(dbg, device_path):
    ...
    if not device_path.startswith(ZVOL_DEV_PREFIX):
        log.debug("%s: not a zvol device, skipping feature detection: %s", dbg, device_path)
        return {}
...
class Implementation(xapi.storage.api.v5.datapath.Datapath_skeleton):
    ...
    def attach(self, dbg, uri, domain):
        ...
        try:
            ...
            log.debug("{}: parsed sr_type={} device_path={}".format(dbg, sr_type, device_path))
        ...
        log.debug(
            "{}: returning {} implementations for {}".format(dbg, len(implementations), device_path)
        )
    ...
    def deactivate(self, dbg, uri, domain):
        ...
        try:
            ...
            log.warning("{}: deactivate URI parse failed (continuing): " "{}".format(dbg, e))
            return
    ...
    def open(self, dbg, uri, persistent):
        ...
        """
        log.debug("{}: raw-qdisk open uri={} persistent={}".format(dbg, uri, persistent))
    ...
    def import_activate(self, dbg, uri, domain):
        ...
        """
        log.debug("{}: raw-qdisk import_activate uri={} domain={}".format(dbg, uri, domain))
                ...
                if existing.scm_proxy_pid is not None and existing.scm_sock_path:
                    try:
                        ...
                        log.debug(
                            ...
                            "SCM proxy pid={}".format(dbg, existing.pid, existing.scm_proxy_pid)
                        )
        ...
        log.debug(
            "{}: import_activate: SCM socket at {} (proxy pid={})".format(dbg, scm_path, proxy_pid)
        )
    ...
    * Otherwise the daemon may legitimately be serving a concurrent
    ...
    for entry_key in entries:
        ...
        try:
            ...
            log.debug("{}: GC: dropping unreadable metadata {}: {}".format(dbg, entry_key, e))
...
def _ensure_qemu_daemon(dbg, key, device_path):
    ...
    if existing:
        try:
            os.kill(existing.pid, 0)
            log.debug(
                "{}: reusing existing qemu-storage-daemon " "pid={}".format(dbg, existing.pid)
            )
            return existing
        except OSError:
            log.debug("{}: cleaning up dead qemu-storage-daemon".format(dbg))
    ...
    try:
        ...
        qemudisk_raw.save_metadata(dbg, key, qemu_disk)
        log.debug(
            "{}: spawned qemu-storage-daemon pid={} for {}".format(dbg, qemu_disk.pid, device_path)
        )
        return qemu_disk
    except Exception as e:  # pylint: disable=broad-exception-caught
        log.error("{}: failed to spawn qemu-storage-daemon for {}: {}".format(dbg, device_path, e))
        return None
...
def _start_tcp_nbd_export(dbg, qemu_disk, key):
    ...
    try:
        ...
        qemu_disk.qmp_command(
            dbg,
            "nbd-server-start",
            addr={"type": "inet", "data": {"host": "0.0.0.0", "port": str(port)}},  # nosec B104
        )
        ...
        log.debug(
            "{}: TCP NBD export started on port {} " "(export='sxm-target')".format(dbg, port)
        )
        ...
        log.warning(
            "{}: failed to start TCP NBD export: {} " "(SCM_RIGHTS fallback remains)".format(dbg, e)
        )
        return None
...
def _find_xenopsd_qemu_for_device(dbg, device_path):
    ...
    if not os.path.isdir(_XEN_QMP_DIR):
        log.debug("{}: xenopsd-qemu discovery: {} missing -- " "no-op".format(dbg, _XEN_QMP_DIR))
        return None

    sockets = sorted(glob.glob(os.path.join(_XEN_QMP_DIR, _XEN_QMP_SOCKET_GLOB)))
    if not sockets:
        log.debug(
            "{}: xenopsd-qemu discovery: no QMP sockets in " "{} -- no-op".format(dbg, _XEN_QMP_DIR)
        )
        return None
    ...
    for sock in sockets:
        try:
            ...
            log.info(
                ...
                "(domid={}, target={})".format(dbg, device_path, sock, domid, target_real)
            )
            return qd
            ...
            log.debug("{}: xenopsd-qemu discovery: {} skipped: " "{}".format(dbg, sock, e))
            continue
        except Exception as e:  # noqa: BLE001  pylint: disable=broad-exception-caught
            log.warning(
                "{}: xenopsd-qemu discovery: unexpected " "error on {}: {}".format(dbg, sock, e)
            )
            continue

    log.debug(
        "{}: xenopsd-qemu discovery: no socket carries " "{} -- no-op".format(dbg, device_path)
    )
    return None
...
def _cbt_persist_on_deactivate(dbg, device_path):
    ...
    if qemu_disk is None:
        log.debug(
            "{}: CBT persist: no qemu instance carries " "vdi={} -- no-op".format(dbg, vdi_key)
        )
        return
    ...
    bitmaps = [bm for bm in all_bitmaps if _is_persistent_tracking_bitmap(bm.get("name"))]
    if not bitmaps:
        if all_bitmaps:
            log.debug(
            ...
            log.debug(
                "{}: CBT persist: no active bitmaps on vdi={} " "-- no-op".format(dbg, vdi_key)
            )
        return
    ...
    try:
        ...
        log.warning("{}: legacy CBT migration failed " "(continuing): {}".format(dbg, e))
    ...
    for bm in bitmaps:
        ...
        try:
            ...
            log.warning("{}: CBT persist: export_bitmap('{}') failed: " "{}".format(dbg, name, e))

    if not payload:
        log.debug("{}: CBT persist: no exportable bitmaps for vdi={}".format(dbg, vdi_key))
        return

    qemudisk_raw.save_cbt_metadata(dbg, sr_uri, vdi_key, payload)
    log.debug("{}: CBT persist: saved {} bitmap(s) for vdi={}".format(dbg, len(payload), vdi_key))
...
def _cbt_restore_on_activate(dbg, device_path):
    ...
    if qemu_disk is None:
        log.debug(
            "{}: CBT restore: no qemu instance carries " "vdi={} -- no-op".format(dbg, vdi_key)
        )
        return
    ...
    if sr_uri is None:
        log.debug(
            "{}: CBT restore: SR URI unresolvable for {}, " "skipping load".format(dbg, device_path)
        )
        return
    ...
    if not isinstance(payload, dict):
        log.warning(
            ...
            "unexpected shape ({}), skipping".format(dbg, vdi_key, type(payload).__name__)
        )
        return
    ...
    log.debug(
        ...
        "(skipped {} non-tracking) for vdi={}".format(dbg, restored, len(payload), skipped, vdi_key)
    )
