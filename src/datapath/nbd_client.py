...
def _recv_exact(sock, n):
    ...
    while len(buf) < n:
        ...
        if not chunk:
            raise NBDProtocolError("short read: wanted {} more bytes, got EOF".format(n - len(buf)))
        buf.extend(chunk)
    return bytes(buf)
...
def _handshake(sock):
    ...
    if magic != NBDMAGIC:
        raise NBDProtocolError("bad NBD magic: 0x{:x} (expected 0x{:x})".format(magic, NBDMAGIC))
...
def _opt_set_meta_context(sock, export_name, context):
    ...
    while True:
        ...
        if rep & NBD_REP_FLAG_ERROR:
            raise NBDProtocolError(
                "NBD_OPT_SET_META_CONTEXT failed: rep=0x{:x} " "body={!r}".format(rep, body)
            )
    ...
    if context_id is None:
        raise NBDProtocolError("server did not assign a context id for {}".format(context))
    return context_id
...
def _opt_go(sock, export_name):
    ...
    while True:
        ...
        if opt != NBD_OPT_GO:
            raise NBDProtocolError("unexpected option in reply: {}".format(opt))
        if rep & NBD_REP_FLAG_ERROR:
            raise NBDProtocolError("NBD_OPT_GO failed: rep=0x{:x} body={!r}".format(rep, body))
        if rep == NBD_REP_INFO:
...
def _read_structured_chunks(sock, expected_handle):
    ...
    while True:
        magic, flags, ctype, handle, length = struct.unpack("!IHHQI", _recv_exact(sock, 20))
    ...
    while offset < export_size:
        ...
        for ctype, body in _read_structured_chunks(sock, handle):
            ...
            if ctype == NBD_REPLY_TYPE_ERROR:
                ...
                raise NBDProtocolError("BLOCK_STATUS error: code={} msg={}".format(err_code, msg))
