...
class QMPConnection:
    ...
    def connect(self, timeout=5.0):
        ...
        for _ in range(50):
            ...
            raise QMPError(
                "Failed to connect to QMP socket: {}".format(self.socket_path)
            )
...
def _detect_block_node(dbg, qmp, device_path=None):
    ...
    for block in blocks:
        ...
        if node and inserted.get("dirty-bitmaps"):
            log.debug(
                "{}: matched block node '{}' by dirty "
                "bitmaps presence".format(dbg, node)
            )
            return node
...
class QemuDiskRaw:
    ...
    def open(self, dbg, device_path):
        ...
        log.debug(
            "{}: device {} already opened by qemu-storage-daemon".format(
                dbg, device_path
            )
        )
    ...
    def mirror_cancel(self, dbg, job_id):
        ...
        try:
            ...
            log.warning(
                "{}: mirror_cancel: failed to cancel job '{}': {}".format(
                    dbg, job_id, e
                )
            )
    ...
    def cbt_bitmap_add(self, dbg, bitmap_name, granularity=65536):
        ...
        """
        log.debug(
            "{}: creating CBT bitmap '{}' granularity={}".format(
                dbg, bitmap_name, granularity
            )
        )
        self.qmp_command(
            dbg,
    ...
    def cbt_bitmap_merge(self, dbg, target_name, source_names):
        ...
        """
        log.debug(
            "{}: merging bitmaps {} into '{}'".format(dbg, source_names, target_name)
        )
    ...
    def cbt_list_bitmaps(self, dbg):
        ...
        log.debug(
            "{}: found {} CBT bitmaps on node {}".format(
                dbg, len(bitmaps), self.block_node_name
            )
        )
        return bitmaps
    ...
    def cbt_snapshot_bitmap(self, dbg, current_name, new_name, granularity=None):
        ...
        """
        log.debug(
            "{}: snapshotting bitmap '{}' -> new '{}'".format(
                dbg, current_name, new_name
            )
        )
        ...
        log.debug(
            ...
            "'{}' active (granularity={})".format(
                dbg, current_name, new_name, granularity
            )
        )
    ...
    def cbt_export_bitmap(self, dbg, bitmap_name, export_name=None):
        ...
        log.debug(
            "{}: creating NBD export '{}' with bitmap '{}'".format(
                dbg, export_name, bitmap_name
            )
        )
    ...
    def prepare_bitmap_from_payload(self, dbg, bitmap_name, payload):
        ...
        **The container-only restore is deliberate, not a gap.**
        ...
        log.debug(
            ...
            "job)".format(
                dbg, bitmap_name, granularity, payload.get("dirty_count", "?")
            )
        )
        ...
        for bm in self.cbt_list_bitmaps(dbg):
            if bm.get("name") == bitmap_name:
                ...
                log.warning(
                    ...
                    "and recreating".format(
                        dbg, bitmap_name, bm.get("granularity"), granularity
                    )
                )
...
def create(dbg, key, device_path):
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "qemu-storage-daemon failed to start: {} {}".format(stdout, stderr)
        )
    ...
    return QemuDiskRaw(
        pid, qmp_sock, key, device_path, nbd_sock, nbd_server_running=True
    )
...
def _cbt_metadata_dir(sr_uri):
    ...
    return os.path.join(
        _sr_mount_from_uri(sr_uri), DRIVER_METADATA_DIRNAME, CBT_METADATA_SUBDIR
    )
...
def migrate_legacy_cbt_metadata(dbg, sr_uri, legacy_key, new_key):
    ...
    if not os.path.exists(new_file):
        log.info(
            "{}: migrating legacy CBT metadata {} -> {} (#104)".format(
                dbg, legacy_file, new_file
            )
        )
