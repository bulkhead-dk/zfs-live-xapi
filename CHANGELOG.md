...
- Fix 4 possibly-unbound / None-access bugs in zfs send|recv pipeline error handlers found by pyright static analysis (#340 / PR #343):
  - xe_wrapper: `recv_proc.kill()` on possibly-unbound variable in cross-host copy exception handler
  - xe_wrapper: `send_proc.stdout.close()` without None guard
  - zfs_operations: `recv.kill()` on possibly-unbound variable in `vol_send_receive` exception handler
  - zfs_operations: `send.stdout.close()` without None guard
- Remove unused `# type: ignore` comment on `xe_wrapper.py:84` (mypy `warn_unused_ignores`)
...
- **`Makefile`** — single entry point for `make test` / `make lint` / `make pre-commit` plus operator-workflow wrappers (`make HOST=... deploy` / `teardown` / `smoke-test` / etc.). Underlying tools stay directly invocable. (#167 / PR #168)
- **`Pipfile`** — pinned dev dependencies (pytest, black, ruff, pylint, mypy, bandit, ansible-lint, yamllint, codespell, pre-commit, ansible) so contributors get the full toolchain in one `pipenv install --dev`. `Pipfile.lock` deliberately gitignored. (#181 / PR #182)
